...
class DataPreprocessingToolkit(object):
    ...
    def add_weekend_stay(df):
        ...
        df["weekend_stay"] = ((start_day in (4, 5)) | (end_day in (4, 5)) | ((end_day < start_day) & (start_day < 6)) | ((df["date_to"] - df["date_from"]).dt.days > 5)).astype("string")
        return df
    ...
    def add_night_price(df):
        ...
        :rtype: pd.DataFrame
        """
        df["night_price"] = np.round(df["accommodation_price"].div(df["length_of_stay"] * df["n_rooms"]), 2)
        return df
    ...
    def aggregate_group_reservations(self, df):
        ...
        sum_columns_df = group_reservations[["group_id"] + self.sum_columns].groupby("group_id").sum()
        # Apply group by on 'group_id' and take the mean in columns given under self.mean_columns
        mean_columns_df = group_reservations[["group_id"] + self.mean_columns].groupby("group_id").mean()
        ...
        mode_columns_df = group_reservations[["group_id"] + self.mode_columns].groupby("group_id").agg(lambda x: x.value_counts().index[0])
        # Apply group by on 'group_id' and take the first value in columns given under self.first_columns
        first_columns_df = group_reservations[["group_id"] + self.first_columns].groupby("group_id").first()
        ...
        agg_group_reservations = pd.concat([sum_columns_df, mean_columns_df, mode_columns_df, first_columns_df], axis=1)
        df = pd.concat([non_group_reservations, agg_group_reservations])
        return df
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        :rtype: pd.DataFrame
        """
        df["room_segment"] = df["night_price"].apply(lambda x: self.map_value_to_bucket(x, self.room_segment_buckets))
        return df
